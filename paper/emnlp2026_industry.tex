...
applying DTLN-AEC~\citep{westhausen2021acoustic} (512-unit, 10.4M
parameters), a dual-signal transformation LSTM network for acoustic
echo cancellation, to the Echo~(sim) microphone signal using the
...
WER reflects Whisper/wav2vec2 accuracy on spontaneous SLURP speech).
...
applies to suppress the far-end signal, changes that are immaterial to intent recognition, but register as severe distortion under SI-SDR and as elevated modulation depth under SRMR. STOI also drops below its Echo~(sim) value (0.47 vs.\ 0.52): STOI's short-time envelope correlation model assumes linear, memoryless distortion, an assumption that nonlinear echo cancellation deliberately violates, so STOI scores can fall even when perceptual intelligibility improves. Note that SI-SDR, SRMR, and STOI are not designed for AEC evaluation, where the output is intentionally gain- and phase-modified relative to the near-end reference. Taken together with the MetricGAN{+} case, these results show
...
condition yields $|\rho| \leq 0.34$ (STOI within MetricGAN{+};
PESQ reaches $|\rho| = 0.26$ at most; SNR, SRMR, and SQUIM-MOS
stay below 0.15; full breakdown in Appendix~\ref{app:within_corr},
...
82.3\,\% of above-median-PESQ clips still diverge, i.e.\ PESQ offers
...
The benchmark uses English-only data (SLURP); enhancement artefacts
...
generation tasks---summarisation, question answering, or dialogue---
...
classifiers such as BERT-style joint intent/slot models, which may
respond differently to transcript noise---remains future work.
...
SE systems remain untested; the rate of intent collapse under those
