...
%  GN Store Nord A/S \\
%  Lautrupjerg 7, 2750 Ballerup, Denmark \\
...
improves downstream task performance. Yet whether SE-induced distortions propagate beyond the transcription level to affect downstream LLM task performance has not been systematically investigated. We introduce Output Divergence Rate (ODR), which measures
...
models, ruling out Whisper-specific artifacts. Our main contributions are as follows:
...
\citet{rai2024denoasr} show that denoising amplifies demographic
bias in ASR.
\citet{ali2022time} propose joint SE+intent training, noting that
independently optimised SE may not serve downstream NLP well.
...
VoiceBench~\citep{chen2024voicebench, chen2026voicebench} evaluates
LLM voice assistants under acoustic variation, but treats the whole
audio-to-response pipeline as a black box rather than isolating the
SE stage.
...
Our benchmark fills this gap: it isolates SE as the variable of
interest, as URGENT does, but measures its effect on an LLM's task output rather than on transcription or signal quality alone.
...
applying DTLN-AEC~\citep{westhausen2021acoustic}(512-unit variant, 10.4M parameters), a dual-signal transformation LSTM network for acoustic
...
applies to suppress the far-end signal, changes that are immaterial to intent recognition, but register as severe distortion under SI-SDR and as elevated modulation depth under SRMR. Note that SI-SDR and SRMR are not designed for AEC evaluation, where the output is intentionally phase- and gain-modified relative to the near-end reference. Taken together with the MetricGAN+ case, these results show
...
\emph{general\_greet}),
revealing distributional shift in LLM input space where SE removes
...
benign correction; MetricGAN{+}'s 7.6\,:\,1 ratio shows its intent
damage is real rather than an artifact of clean-speech classification
errors.
...
41.2\,\% of above-median-PESQ clips still diverge, i.e.\ PESQ offers
no safety margin precisely when damage is worst; under Dereverb and
Noisy, 42--44\,\% of below-median-PESQ clips do \emph{not} diverge,
i.e.\ PESQ over-warns on clips the LLM handles correctly. No single
...
Of 4{,}149 divergent clips, 90.4\% represent domain-level shifts and
...
\textbf{Single LLM.}
Our primary results use Gemini~2.5 Flash Lite. A preliminary
replication with Gemini~2.5 Pro (reasoning-enabled) on two
conditions confirms the central finding (MetricGAN{+} doubles
ODR vs.\ noise, 1.91$\times$ ratio); however, full multi-family
LLM evaluation remains future work.
 
\textbf{AEC scope.}
The echo condition includes both residual echo and a single
neural AEC system (DTLN-AEC); other AEC architectures and
multi-microphone setups remain untested.

\textbf{Clean-reference requirement.}
Like PESQ and STOI, ODR requires a clean reference recording per
clip, which is typically unavailable at deployment time. ODR is
therefore intended as an offline, development-time criterion for
comparing SE front-ends before deployment, not an online per-utterance
monitor; reference-free proxies (e.g.\ transcript stability across
paraphrase, or ASR confidence) are a direction for future work.

\textbf{SE method coverage.}
We test one discriminative GAN-based suppressor (MetricGAN{+});
diffusion-based, self-supervised, and commercial SE systems remain
untested and may show different ODR profiles.

\textbf{Classifier architecture.}
We use a single generative LLM classifier (Gemini~2.5 Flash Lite);
encoder-based intent classifiers (e.g.\ BERT-style joint intent/slot
models) may exhibit different sensitivity to SE-induced transcript
noise, and conclusions could differ under such architectures.

\textbf{Entity-level effects.}
ODR is computed over closed-set intent labels only; our error
taxonomy does not separately track named-entity or slot-level
errors, since the SLURP test release used here does not expose
entity annotations. Intent labels are typically domain/action pairs
that survive many entity-level ASR substitutions (e.g.\ mishearing a
city or contact name), so ODR may understate SE's impact on tasks
that depend on entity fidelity, such as slot filling or named-entity
recognition; extending ODR to slot-level F1 is future work.
...
(\S\ref{sec:results}).
\input{tables/table6_correction_regression}
...
\input{tables/table7_metric_failure}
